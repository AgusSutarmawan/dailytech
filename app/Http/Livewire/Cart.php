        ...
        $kembalian = (int) $bayar - (int) $cartTotal;
         ...
         //if($kembalian >= 0){
            ...
            try {
                ...
                $this->dispatchBrowserEvent('alert',[
                    'type'=>'error',
                    'message'=>"Something goes wrong while ordering!!"
                ]);
                $this->hideModal();
            }
            }catch(\Exception $e){
         ...
         //     session()->flash('success', 'Successfully Ordered');
           //     $this->hideModal();
            ...
            // } catch (\Throwable $th) {
            //     DB::rollback();
            //     // return view()->back()->with('error', 'Terdapat kesalahan, data telah di Rollback!');
            //     return session()->flash('error', $th);
            // }
            // return view('/livewire/cart',)->with('success', 'Berhasil dipesan');
        // }
